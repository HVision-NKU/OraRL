...
class FSDPVLLMShardingManager(BaseShardingManager):
    def __init__(
        ...
        # Match Accelerate's ``set_seed(seed, device_specific=True)`` convention
        # used by the reference trainer: each data-parallel generation worker
        # starts from base_seed + rank, while all TP ranks agree.
        gen_dp_rank = self.device_mesh["dp"].get_local_rank()
        torch.cuda.manual_seed(int(rollout_seed) + gen_dp_rank)
