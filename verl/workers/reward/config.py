...
class RewardConfig:
    ...
    reward_function_is_module: bool = field(default=False, init=False)
    ...
    def post_init(self):
        if self.reward_function is not None:  # support custom reward function, e.g., ./math.py:main
            ...
            self.reward_function_is_module = bool(
                re.fullmatch(
                    r"[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*",
                    self.reward_function,
                )
            )
            if not self.reward_function_is_module:
                self.reward_function = get_abs_path(
                    self.reward_function,
